...
agent_summary: "v0.17 alpha: communications module, unified templates, forms removed, API tokens, service account removal on main, deep /ready, cron functions, Node 24/pnpm, Admin MCP, GitOps export/import."
---
...
## On main after v0.17.0-alpha.3

These breaking changes are on the v0.17 track (`main`) and will ship in the next alpha tag:

- **authentication:** Legacy **service account** login removed — superseded by core **`cdt_` API tokens** (`POST /api-tokens`); `Service` collection migrated away on startup

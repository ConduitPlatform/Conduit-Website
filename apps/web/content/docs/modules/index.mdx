...
agent_summary: "Index of first-party modules: eight stable v0.17 modules plus opt-in embeddings. Embeddings is not in standalone."
---

**Eight stable v0.17 modules plus opt-in embeddings.** Embeddings is not included in standalone. Legacy **email**, **sms**, and **push-notifications** are consolidated into **communications**; the **forms** module is removed — use database schemas, custom endpoints, or the functions module instead.
...
<ModuleGrid>
  ...
  <ModuleCard title="Embeddings" href="/docs/modules/embeddings" description="Opt-in text-to-vector generation, backfills, and text-in semantic search" />
...
| **Embeddings** | Database (Vector fields, indexes, raw-vector search). HTTPS provider required for generation/search, not process startup. | — |
| **Storage** | Database | — |
| **Communications** | — | — |

...
<ModuleCapabilities
  items={[
    ...
    "Indexes",
    "Vector fields & indexes",
    "Populate relations",
    "Text search (like: true)",
    "Database introspection (Admin API)",
    "GitOps export (schemas, extensions, endpoints)",
    "GraphQL (via router)",
    "Raw-vector search (Admin API)",
  ]}
/>

Vector fields (`type: Vector`, `dimensions`, `similarity`), vector indexes, Database vector capabilities, and **raw-vector** search (`POST /database/schemas/:schemaName/vector-search`) belong to this module. Text-to-vector generation, embedding configs, backfills, and **text-in** search belong to [Embeddings](/docs/modules/embeddings). Deleting an embedding config does **not** drop vector fields or indexes.
...
<NextSteps steps={[
  ...
  { title: "Embeddings (text-in search)", href: "/docs/modules/embeddings" },

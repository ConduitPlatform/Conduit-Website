...
### Machine auth (no service accounts)

v0.17 does **not** recommend Client API service accounts (`POST /authentication/service`). For automation, CI, and MCP clients, use **[Admin API tokens](/docs/reference/admin-api)**:

- Create `cdt_` API tokens via Admin API or MCP
...
Service-account login remains in the codebase behind `service.enabled` for legacy deployments; new integrations should use Admin API tokens.
</ModuleDeepDive>

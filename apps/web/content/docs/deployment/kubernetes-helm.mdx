...
agent_summary: "helm install conduit conduit-platform/conduit; core readiness HTTP :3030/ready; liveness gRPC :5000; READY_REQUIRED_MODULES set by chart; embeddings via install.embeddings.enabled (default false)."
---
...
The chart flag `install.embeddings.enabled` defaults to **`false`**. Setting it to `true` deploys the embeddings **pod** only. It does **not** set module convict `enabled` — workers, mutation subscriptions, and search stay off until you patch Core config (`patch_config_embeddings`).

Pin the chart / image to a **published** embeddings tag. Do not enable the workload against `latest` until that tag exists. Standalone does not include this module. Rollout order, capability checks, and rollback: [Embeddings](/docs/modules/embeddings).

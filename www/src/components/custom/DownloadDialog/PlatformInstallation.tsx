...
const PlatformInstallation: FC<Props> = ({ platform }) => {
  ...
  const platformInfo: Map<Platform, PlatformInstallInfo> = new Map([
    ...
    [Platform.NPM, {
      installLines: [['npx -y @conduitplatform/cli@latest deploy setup --config', cliInstallText]],
      bottomText: <strong hidden={window.navigator.platform !== 'Win32'}>Windows users should run through WSL</strong>
    }],
    [Platform.HELM, {
      installLines: [
        ['helm repo add conduit-platform https://conduitplatform.github.io/helm-charts', 'Add Helm Repo'],
        ['helm repo update'],
  ...
  activeInfo.installLines.forEach((line, index) => {
    ...
    lineElements.push(
      <Box display="flex" flexDirection="column">
        { line[1] && <Typography textAlign="center" variant="caption" marginTop={2}>{line[1]}</Typography> }
  ...
  return (
    <Box display="flex" alignItems="center" flexDirection="column" gap={2}>
      <Box display="flex" flexDirection="column">
        {lineElements}
        {activeInfo.bottomText && <Typography textAlign="center">{activeInfo.bottomText}</Typography>}
      </Box>

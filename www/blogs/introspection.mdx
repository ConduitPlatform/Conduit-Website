...
<p>
    You see, the main problem remains that most users that will want to use Conduit, will also want to store actual test
...
imported before persisting them into the Conduit DB. The neat part in this process is that not only you save a lot of time, but the same database that stores all
...
<br/>
However this wasn't the case for Mongo Databases. Since MongoDB is a No-SQL Database the concept of schema differs: there is not a schema definition stored somewhere where we
...
seamless experience. You can always provide your opinion on what else we can add to make schema imports even better [here](https://github.com/ConduitPlatform/Conduit/issues/77).

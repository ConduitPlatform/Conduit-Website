...
|   Route                    | Method |
| :------------------------: | :----: |
| /admin/cms/customEndpoints |  POST  |
...
[Create](../../1_get-started.mdx/#schemas-example) and [populate](../../1_get-started.mdx/#documents-example)
a schema named `citizens`, then send a **POST** request, including the necessary [authorization headers for an admin request] (TODO).

```json title="POST on /admin/cms/customEndpoints"
{
  "name": "AdultSam",
  "operation": 0,
...
It can be accessed through a GET request on *"/cms/function/AdultSam"*.
...
|   Route                        | Method  |
| :----------------------------: | :-----: |
| /admin/cms/customEndpoints/:id |  PATCH  |

...
```
***It's important to say that the JSON Admin Token could be retrieved only by making the request to the REST API.<br />
And that's because the Admin GraphQL API is not implemented yet.<br />***
...
There are three client headers.**Client Id, Client Secret, Authorization**.<br />

### Authorization

Actually the Authorization Header is a Bearer Token. It is mandatory to include it in each request.<br />
So as to fetch the token you need to make a POST request to [/authentication/local](../modules/authentication/api_reference/user/local#login).
...
curl --location --request GET 'CONDUIT_URL/chat/rooms' \
--header 'masterkey: M4ST3RK3Y' \
--header 'Content-Type: application/json' \

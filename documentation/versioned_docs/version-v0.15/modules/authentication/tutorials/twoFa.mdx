...
To enable 2FA, users must previously have successfully logged in to their account.
...
The fist time a user enables two factor authenticator has an extra step.
They must verify the 6-digit code that was sent to their phone number or authenticator app.
...
From now on, the endpoint that they will use to login is the following:
...
The codes in the authenticator app are generated automatically.
...
### Generate Backup Codes and Recovery Codes

If a user has no access to their phone or authenticator app, they can use the backup codes to login.<br />
**The backup codes are only used once and a user must generate new ones before using all of them.**

```bash title="Generate backup codes"
curl --location --request GET '//authentication/twoFa/generate' \
--header 'Accept: application/json' \
--header 'Authorization: Bearer ' \
--header 'masterkey: <API Key>'
```
...
curl --location --request POST '//authentication/twoFa/recover' \
--header 'Content-Type: application/json' \
--header 'Accept: application/json' \
--header 'Authorization: Bearer ' \
--header 'masterkey: <API Key>' \
...
```
After successfully recovering 2FA, access and refresh tokens are returned as well as a message that informs the user about the back-up codes that have left.
...
curl --location --request PUT '//authentication/twoFa/disable' \
--header 'Accept: application/json' \
--header 'Authorization: Bearer {{token}}' \
--header 'masterkey: <API Key>'

```
After successfully disabling 2FA, a message is returned that informs the user that 2FA has been disabled.

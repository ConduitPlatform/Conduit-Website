...
```ts title="Request"
await grpcSdk.pushNotifications.setNotificationToken({
...
```ts title="Request"
await grpcSdk.pushNotifications.getNotificationTokens({
    userId: "someuserid", // user related with this token
})
```
...
```ts title="Request"
await grpcSdk.pushNotifications.sendNotification({
    sendTo: "diresuemos",
...
```ts title="Request"
await grpcSdk.pushNotifications.sendToManyDevices({
